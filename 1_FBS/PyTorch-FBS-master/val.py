...
def main():
    ...
    if os.path.isfile(log_path):
        os.remove(log_path)
    logger.add(log_path)
    net = resnet34(num_classes=10).cuda(cuda1)
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=0.4, momentum=0.9, weight_decay=0.0001)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [30, 60, 80], 0.2)
    loss_function = torch.nn.CrossEntropyLoss()
    best_accuracy = 0

    checkpoint = torch.load("ckpts/0.4-latest.pth")
